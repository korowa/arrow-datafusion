        ...
        loop {
            match &self.state {
                ...
                SMJState::JoinOutput => {
                    ...
                    // TODO: check for freezes/whatever -- is this case even possible
                    // why not streamed += 1?
...
impl SMJStream {
    ...
    fn join_partial(&mut self) -> Result<()> {
        ...
        let last_buffered_pass = self.current_ordering == Ordering::Greater && !self.buffered_joined;
...
mod tests {
    ...
    async fn join_right_spill() -> Result<()> {
        ...
        let right = build_table_from_batches(vec![right_batch_1, right_batch_2, right_batch_3]);
        let on = vec![(
            Column::new_with_schema("b1", &left.schema())?,
            Column::new_with_schema("b2", &right.schema())?,
        )];
    ...
    async fn join_left_semi_spill() -> Result<()> {
        ...
        let (_, batches) = spilled_join_collect(left, right, on, JoinType::LeftSemi).await?;
        let expected = vec![
            "+----+----+----+",
            "| a1 | b1 | c1 |",
            "+----+----+----+",
    ...
    async fn join_left_anti_spill() -> Result<()> {
        ...
        let (_, batches) = spilled_join_collect(left, right, on, JoinType::LeftAnti).await?;
        let expected = vec![
            "+----+----+----+",
            "| a1 | b1 | c1 |",
            "+----+----+----+",
